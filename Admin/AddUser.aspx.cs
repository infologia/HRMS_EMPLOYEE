...
public partial class Admin_AddUser : System.Web.UI.Page
{
    ...
    public static string[] GetCustomers(string prefix, string prefix2)
    {
        ...
        using (SqlConnection conn = new SqlConnection())
        {
            ...
            {
                cmd.CommandText = "Select Firstname,Employeekey from IT_EmployeeRegister where Employeekey NOT IN (SELECT Employeekey FROM tt_adduser WHERE projectkey=@Projectkey) and roles=1 and Firstname like @searchtext+ '%'";
    ...
    protected void btn_submit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (dt_chk.Rows.Count > 0)
            {
                ...
                if (str_stus == "1")
                {
                    string str_addpjt = "insert into TT_AddUser(Projectkey,Employeekey,Createdby)values(@Projectkey,@Employeekey,@Createdby)";
                    SqlCommand cmd2 = new SqlCommand(str_addpjt);
                    cmd2.Parameters.AddWithValue("@Projectkey", ddl_pjtname.SelectedValue);
                    cmd2.Parameters.AddWithValue("@Employeekey", ddl_employeelist.SelectedValue);
                    cmd2.Parameters.AddWithValue("@Createdby", str_userkey);
                    DA.ExecuteNonQuery(cmd2);
                    string str_username = "Select username,email from IT_EmployeeRegister where  Employeekey='" + ddl_employeelist.SelectedValue + "' ";
    ...
    {
        string str_pjtkey = ddl_pjtname.SelectedValue;
        if (str_pjtkey!=null && str_pjtkey != "" && str_pjtkey != "0")
        {
            Guid parsedGuid = Guid.Parse(str_pjtkey);

        }
        else
        {
            ClientScript.RegisterStartupScript(this.GetType(), "Internal Tool", "<script>alert('Project key not working');</script>");
            return;
        }

            string str_cat = "Select Firstname,Employeekey from IT_EmployeeRegister where Employeekey NOT IN (SELECT Employeekey FROM tt_adduser WHERE projectkey='" + str_pjtkey + "') and roles=1";
        ...
        if (ds != null && ds.Tables.Count > 0)
        {
            ddl_employeelist.DataSource = ds.Tables[0];
            ddl_employeelist.DataTextField = "Firstname";
            ddl_employeelist.DataValueField = "Employeekey";
            ddl_employeelist.DataBind();
            ddl_employeelist.Items.Add(new ListItem("Select Employee", "0"));
            ddl_employeelist.SelectedValue = "0";
        }
    }
}
